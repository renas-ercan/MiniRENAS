...
class Application(dbus.service.Object):
    ...
    def __init__(self, bus, spi):
        ...
        print("Application initialised, service added")
...
class TestCharacteristic(dbus.service.Object):
        ...
        if message == 'start':
            ...
            try:
                ...
                with self.spi_lock:
                    ...
                    self.spi.close()
                print(f"Integration time set to {integration_time_ms} ms, SPI values: LSB={lsb:#06x}, MSB={msb:#06x}")
            except ValueError:
    ...
    def read_data(self):
        ...
        while not self.reading_thread_event.is_set():
            ...
            print(f"Received data from SPI: {data}")
            ...
            print(f"Temperature_data: {temperature_data}")
            # Combine spectral data and temperature data
            combined_data = bytes(data) + temperature_data
            GLib.idle_add(self.send_notification, combined_data)

    def send_notification(self, combined_data):
        # Send the combined data in chunks
        PACKET_TYPE_DATA_CHUNK = 0x01
        self.send_data_in_chunks(combined_data, PACKET_TYPE_DATA_CHUNK)
        return False
    ...
    def read_reference_data(self):
        ...
        print(f"Received reference data from SPI: {data}")
        print(f"Temperature value: {temperature_value}")
        ...
        # Combine spectral data and temperature data
        combined_data = bytes(data) + temperature_data
        GLib.idle_add(self.send_reference_data, combined_data)

    def send_reference_data(self, combined_data):
        # Send the reference data in chunks
        PACKET_TYPE_REFERENCE_DATA = 0x03
        self.send_data_in_chunks(combined_data, PACKET_TYPE_REFERENCE_DATA)
        return False

    def send_data_in_chunks(self, combined_data, packet_type):
        '''Split the data into chunks and send over BLE'''
        CHUNK_SIZE = 182  # Adjust according to MTU and BLE limitations
    ...
    def send_chunk_notification(self, packet):
        if self.notifying:
            ...
            print(f"Sending chunk {packet[1]+1}/{packet[2]} with length {len(value)}")
...
def main():
    ...
    advertisement_manager.RegisterAdvertisement(adv.get_path(), {},
                                                reply_handler=lambda: print('Advertisement registered'),
                                                error_handler=lambda error: print('Failed to register ad:', error))
